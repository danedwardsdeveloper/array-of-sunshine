  ...
  return (
    <ArticleLayout article={article}>
      <Paragraph>
        ...
        by Robert C. Martin is probably the closest a software development book can get towards being a
        timeless classic. Published in 2008, it emphasizes writing readable, maintainable, and elegant code.
      </Paragraph>
      <Paragraph>
      ...
      <Paragraph>
        Martin stresses the importance of choosing clear, intention-revealing names for variables, functions,
        and classes. Well-chosen, expressive names can make code self-explanatory, reduce the need for
        comments, and reduce the number of things you need to remember.
      </Paragraph>
      <Paragraph>
      ...
      <Paragraph>
        ...
        <InlineCode>e</InlineCode> could mean <InlineCode>error</InlineCode>, <InlineCode>event</InlineCode>,
        or <InlineCode>evaluate</InlineCode>, so it can be confusing for someone (your future self included)
        reading your code.
      </Paragraph>
...
function getFlaggedCells(gameBoard: Cell[]): Cell[] {
    return gameBoard.filter(cell => cell.isFlagged());
      ...
      <Paragraph>
        Martin advocates for small, focused functions that do one thing well. He provides guidelines on
        function length, parameter lists, and the Single Responsibility Principle.
      </Paragraph>
      <CodeBlock language="typescript" fileName="badExample.ts">
    ...
    if (e.isActive()) {
      ...
      <Paragraph>
        This function violates the Single Responsibility Principle by handling multiple concerns: calculation,
        database operations, document generation, and email notifications.
      </Paragraph>
      <CodeBlock language="typescript" fileName="improvedExample.ts">
...
function notifyInactive Employee(employee: Employee): void {
    sendEmail(employee.getEmailAddress(), "You are not an active employee");
      ...
      <Paragraph>
        This improved version breaks down the colossal function into smaller, more focused functions. Each
        function has a single responsibility, making the code more modular and easier to maintain.
      </Paragraph>
      <Heading2>3. Comments</Heading2>
      <Paragraph>
        While not dismissing comments entirely, Martin argues for code that is so clear and expressive that it
        requires minimal additional explanation. He distinguishes between necessary clarifications and
        redundant noise.
      </Paragraph>
      <CodeBlock language="typescript" fileName="badExample.ts">
...
if (user.isLoggedIn()) {
    // ...
      ...
      <Paragraph>{`This comment is unnecessary as the code clearly expresses what's being checked.`}</Paragraph>
      <CodeBlock language="typescript" fileName="improvedExample.ts">
...
function processUserData(users: User[]): void {
    for (const user of users) {
      ...
      <Paragraph>
        ...
        is using JSDoc comments. You can put these in your types and interfaces, creating helpful popups when
        filling your variables with data.
      </Paragraph>
...
date: string;
      ...
      <Paragraph>
        Martin emphasizes the importance of proper error handling techniques, writing code that gracefully
    ...
    try {
      ...
      <Paragraph>
        This example catches all errors and logs a generic message, losing important error details and
        potentially hiding serious issues.
      </Paragraph>
      <CodeBlock language="typescript" fileName="improvedExample.ts">
...
async function readFile(filename: string): Promise<string> {
    try {
      ...
      <Paragraph>
        This improved version handles specific error types, provides more detailed error messages, and
        correctly propagates errors up the call stack.
      </Paragraph>
      <Heading2>6. Testing</Heading2>
      <Paragraph>
        Martin strongly advocates Test-Driven Development (TDD) and dedicates several chapters to writing
        effective unit tests. For TypeScript projects, I love using Vitest, as you can also write your tests
        in TypeScript with minimal or no additional configuration.
      </Paragraph>
...
describe('Calculator', () => {
    ...
    it('should add two numbers correctly', () => {
      ...
      <Paragraph>
        This test is clear and concise and covers multiple scenarios for the add function, including positive,
        negative, and zero-sum cases.
      </Paragraph>
      ...
      <Paragraph>
        When software is difficult to change because every modification affects many other parts of the
        system.
      </Paragraph>
      <CodeBlock language="typescript" fileName="badExample.ts">
        {`class Report {
    ...
    // ... other methods
      ...
      <Paragraph>
        This class is rigid because any change to the report generation process requires modifying this class,
        potentially affecting all its functionalities.
      </Paragraph>
      <CodeBlock language="typescript" fileName="improvedExample.ts">
        {`interface Data {}
...
class Report {
    ...
    generateReport(): void {
      ...
      <Paragraph>
        This version is more flexible as each component can be modified or replaced independently, adhering to
    ...
    private updateUserCount(): void {
      ...
      <Paragraph>
        If the email sending fails, it will prevent the user count from being updated, even though these
        operations are not logically dependent.
      </Paragraph>
      <CodeBlock language="typescript" fileName="improvedExample.ts">
        {`interface EmailService {
