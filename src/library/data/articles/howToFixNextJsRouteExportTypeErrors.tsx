...
import { november, tags } from "@/library/constants";
import { createDate } from "@/library/utilities";
import type { Article } from "@/types";
import featuredImage from "@public/images/how-to-fix-next-js-15-route-export-type-errors.png";

export const howToFixNextJsRouteExportTypeErrors: Article = {
	publishedAt: createDate(11, november, 2024),
	displayTitle: "How to fix Next.js 15 route export type errors",
	metaTitle: "How to fix Next.js 15 route export type errors",
	metaDescription:
	...
	content: (
		<>
			<p>
				When working with Next.js 15 API routes, you might encounter a
				TypeScript error stating that your route has an invalid export.
				Specifically, you might see this error during the build:
			</p>
			<CodeBlock
				language="bash"
...
Type error: Route "src/app/api/articles/[slug]/route.ts" has an invalid "GET" export:
  Type "Props" is not a valid type for the function's second argument.`}</CodeBlock>
			<p>
				This error occurs when using the traditional approach to typing route
				parameters:
			</p>
			<CodeBlock
   ...
   // error handling
		...
		`}
				...
				<li>
					...
					You must await the <code>params</code> before accessing any parameter
					properties:
					<CodeBlock
						language="typescript"
						fileName="route.ts"
...
const resolvedParams = await params`}</CodeBlock>
			...
			<h2>Verification</h2>
			<p>
				After implementing these changes, your Next.js build should be completed
				successfully. You should see output similar to:
			</p>
			<CodeBlock
				language="bash"
				fileName="Command line"
...
dan@Dans-MacBook-Pro incremental-regeneration % `}</CodeBlock>
			...
			<ul>
				...
				<li>
					The Promise typing allows for better handling of asynchronous route
					parameter resolution
				</li>
				<li>
					Make sure to handle Promise rejection cases in your error handling
					properly
				</li>
				<li>
