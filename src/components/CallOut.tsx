...
export default function CallOut({ colour = 'blue', children, disableEmoji }: CallOutProps) {
  return (
    <div className={clsx('my-4 p-4', 'rounded-lg border-2', 'flex gap-2', colourMap[colour])}>
      {!disableEmoji && (
        <span className="text-xl" role="img" aria-label={`${colour} callout icon`}>
          {emojiMap[colour]}
        </span>
      )}
